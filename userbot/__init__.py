...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        format="%(asctime)s - @noxusbot - %(levelname)s - %(message)s",
        level=DEBUG,
    )
else:
    basicConfig(format="%(asctime)s - @noxusbot - %(levelname)s - %(message)s",
                level=INFO)
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/noxuserbot/noxusbot.git")

# Ayrıntılı konsol günlügü
CONSOLE_LOGGER_VERBOSE = sb(os.environ.get("CONSOLE_LOGGER_VERBOSE", "False"))

# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///noxus.db")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@noxusbot | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "@noxusbot Paketi")
...
with bot:
    if OTOMATIK_KATILMA:
        try:
            bot(JoinChannelRequest("@noxusbot"))
            bot(JoinChannelRequest("@noxusbot"))
        except:
            pass
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @noxusbot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Noxus açabilirsin; Kanala bak` @noxusbot')
            else:
                await event.reply(f'`Eat Noxus s Dick`')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@noxusbot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text=f"**🐺 Pembe Amcıklar AŞKINA! NoxuserBOT!** [Noxus](https://t.me/Noxus_0) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@noxusbot",
                    text="""@noxusbot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/noxusbot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/noxusbot")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/noxuserbot/noxusbot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @noxusbot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"**🐺 Pembe Amcıklar AŞKINA!!** [Noxus](https://t.me/Noxus_0) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
