...
from . import BRAIN_CHECKER, LOGS, bot, PLUGIN_CHANNEL_ID, CMD_HELP, LANGUAGE, NOXUS_VERSION, PATTERNS
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        for Command in Pattern:
            ...
            # NoxusPY
            Noxuspy = re.search('\"\"\"NOXUSPY(.*)\"\"\"', FileRead, re.DOTALL)
            if not Noxuspy == None:
                Noxuspy = Noxuspy.group(0)
                for Satir in Noxuspy.splitlines():
...
LOGS.info("Botunuz çalışıyor! Herhangi bir sohbete .alive yazarak Test edin."
          " Yardıma ihtiyacınız varsa, Destek grubumuza gelin t.me/Noxus_0")
LOGS.info(f"Bot sürümünüz: Noxus {NOXUS_VERSION}")
